...
class Search extends Component {
  ...
  searchData = e => {
    ...
    console.log("2332323232", queryResult)
  ...
  handleKeyDown = (event) => {
    ...
    console.log("ttttttttttt", searchResults)
              ...
              {queryResults.map((item, idx) => {
                return (
                  <TableRowWraper key={`row_${item.isbn}`} data={idx === index ? '#E5E5E5': 'white'}>
                    <TableDataWraper>
                      <Link style={{ color: "black", display: "block" }} href={{pathname: `stocks/${item.tickle.toLowerCase()}`, query: {idx: idx}}}><a>{item.Name}</a></Link>
                    </TableDataWraper>
                    <TableDataWraper>
                      <Link style={{ color: "black", display: "block" }} href={{pathname:`stocks/${item.tickle.toLowerCase()}`, query: {idx: idx}}}><a>{item.tickle}</a></Link>
                    </TableDataWraper>
                  </TableRowWraper>
                )
              })}
